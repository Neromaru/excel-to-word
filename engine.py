...
class TemplateGenerator(object):
    ...
    def __init__(self, path_to_data=None, path_to_folder=None, where_to_save=None, named_header=None):
    ...
    def _serialize_datetimes(self, dict_fields: dict) -> dict:
        ...
        for key, value in dict_fields.items():
            try:
                dict_fields[key] = dt.datetime.strptime(value, '%Y-%m-%d %H:%M:%S').strftime('%d.%m.%Y')
            except:
                continue
    ...
    def generate_templates(self):
        for idx, row in self.excel.iterrows():
            ...
            if os.path.exists(write_folder):
                write_folder = os.path.join(self.save_folder, str(row[self.named_header]) + str(uuid.uuid4())[:8])
            os.mkdir(write_folder)
            for template in self.list_templates():
                template_docx = MailMerge(template)
                fields = {variable:str(row[variable]) for variable in template_docx.get_merge_fields()}
